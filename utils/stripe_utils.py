...
class StripeUtils(object):
    ...
    @staticmethod
    def get_stripe_plan_name(cpu, memory, ssd_size, hdd_size=0):
        """
        Returns the Stripe plan name
        :return:
        """
        if hdd_size == 0:
            return "{cpu} Cores, {memory} GB RAM, {ssd_size} GB SSD".format(
                cpu=cpu, memory=memory, ssd_size=ssd_size
            )
        else:
            return """{cpu} Cores, {memory} GB RAM, {ssd_size} GB SSD,
